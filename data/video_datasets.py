...
class VideoDataset(Dataset):
    def __init__(self, idx_root, frame_root, use_cuda=False, transform=None):
        ...
        self.video_list = [
            name for name in os.listdir(self.idx_root) if os.path.isdir(os.path.join(self.idx_root, name))
        ]
    ...
    def __getitem__(self, item):
        ...
        if self.transform:
            if c == 3:
                frames = torch.cat(
                    [
                        self.transform(io.imread(os.path.join(v_dir, ("%03d" % i) + ".jpg"))).unsqueeze(1)
                        for i in frame_idx
                    ],
                    1,
                )
                ...
                frames = torch.cat(
                    [
                        self.transform(
                            np.expand_dims(io.imread(os.path.join(v_dir, ("%03d" % i) + ".jpg")), axis=2)
                        ).unsqueeze(1)
                        for i in frame_idx
                    ],
                    1,
            ...
            if c == 3:
                frames = torch.cat(
                    [
                        tmp_frame_trans(io.imread(os.path.join(v_dir, ("%03d" % i) + ".jpg"))).unsqueeze(1)
                        for i in frame_idx
                    ],
                    1,
                )
...
class VideoDatasetOneDir(Dataset):
    def __init__(self, idx_dir, frame_root, is_testing=False, use_cuda=False, transform=None):
        ...
        self.frame_root = frame_root
        self.idx_name_list = [
            name for name in os.listdir(self.idx_dir) if os.path.isfile(os.path.join(self.idx_dir, name))
        ]
    ...
    def __getitem__(self, item):
        ...
        if self.transform:
            frames = torch.cat(
                [
                    self.transform(io.imread(os.path.join(v_dir, ("%03d" % i) + ".jpg")).reshape(h, w, c)).resize_(
                        c, 1, h, w
                    )
                    for i in frame_idx
                ],
