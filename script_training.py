...
saving_model_path = os.path.join(
    saving_root, "model_" + model_setting + "/"
)  # './models/model_Memae...epoch_0100_final.pt/'
...
height = width = 128  # only used when changing batch view, images are already downscaled
...
tr_data_loader = DataLoader(video_dataset, batch_size=batch_size_in, shuffle=True, num_workers=opt.NumWorker)
...
# Moving the model to the correct device & initialise optimiser
model.to(device)
...
for epoch_idx in range(start_epoch, max_epoch_num):
    for batch_idx, (_, frames) in enumerate(tr_data_loader):
        ...
        frames = frames.view(
            frames.size(0), chnum_in_, framenum_in_, height, width
...
torch.save(
    model.state_dict(),
    "%s/%s_epoch_%04d_final.pt" % (saving_model_path, model_setting, epoch_idx),
)
